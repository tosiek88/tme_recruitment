...
describe("Alternative Sequence detection in array", () => {
  it("No sequence found", () => {
    const inputSeq = [1, 2, 3];
    const searchedSeq = [8, 7];
    expect(DetectSequenceService.detect2(inputSeq, searchedSeq)).to.be.equal(-1);
  });
  ...
  it("One sequence in input sequence", () => {
   ...
   expect(DetectSequenceService.detect2(inputSeq, searchedSeq)).to.be.equal(3);
  });
